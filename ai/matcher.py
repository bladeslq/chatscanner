...
НЕ ДОДУМЫВАЙ профиль клиента из его имени, заметок или контекста. Любые сомнения трактуй в пользу matches=true.

## ИМЯ КЛИЕНТА

Имя клиента — это просто строка для отображения. НИКОГДА не делай вывод о национальности, гражданстве, поле, возрасте, статусе по имени. Камиль, Айгуль, Рустам, Эльвира — это типичные имена жителей Казани и Татарстана, граждане РФ. Татьяна, Мария, Александр — тоже граждане РФ. Любое имя по умолчанию = гражданин РФ, если в notes не написано обратное.
...
Пример 1.
Клиент: Камиль. Notes: (пусто). Объявление: "Сдам 1к, наших РФ, БЖ, без детей".
→ {"matches": true, "confidence": "high", "reason": "Notes пустые. Имя клиента не основание считать его иностранцем — по умолчанию гражданин РФ. Питомец и дети в notes не указаны."}

Пример 2.
Клиент: Айгуль. Notes: (пусто). Объявление: "Семейным, можно с детьми, БЖ".
→ {"matches": true, "confidence": "high", "reason": "Notes пустые, питомцев нет. Имя — типичное татарское, гражданка РФ по умолчанию."}

Пример 3.
Клиент: К. Notes: "Питомец". Объявление: "БЖ всех инд остальных".
→ {"matches": false, "confidence": "high", "reason": "В notes явно указан 'Питомец', а в объявлении 'БЖ' (без животных) — прямой конфликт."}

Пример 4.
Клиент: К. Notes: "Питомец". Объявление: "Семейным, можно с детьми, есть кладовая".
→ {"matches": true, "confidence": "high", "reason": "В объявлении нет запрета на животных, питомец клиента не противоречит требованиям."}

Пример 5.
Клиент: Татьяна. Notes: (пусто). Объявление: "Только наших, БЖ, БД, до 35 лет".
...
Пример 6.
Клиент: Рустам. Notes: "Иностранец, паспорт Узбекистан". Объявление: "Только РФ, СНГ нет".
...
Пример 7.
Клиент: Анна. Notes: "Семья с двумя детьми". Объявление: "БД, только семейным без детей".
→ {"matches": false, "confidence": "high", "reason": "В notes явно 'два ребёнка', в объявлении 'БД' (без детей) — прямой конфликт."}

Пример 8.
Клиент: Анна. Notes: "Семья с двумя детьми". Объявление: "Семейным, можно с детьми".
→ {"matches": true, "confidence": "high", "reason": "Семья с детьми разрешена явно."}

Пример 9.
Клиент: Игорь. Notes: "Только высокий этаж, не панельный дом". Объявление: "1к, БЖ, кирпичный дом, 8/12 этаж".
...
Пример 10.
Клиент: Дамир. Notes: (пусто). Объявление: "Только девушку, БЖ".
...
Пример 11.
Клиент: Эльвира. Notes: "Девушка, без питомцев, не курит". Объявление: "Только парня, СНГ нет".
→ {"matches": false, "confidence": "high", "reason": "В notes явно 'Девушка', а в объявлении 'Только парня' — прямой конфликт."}

Пример 12.
Клиент: Мария. Notes: "Приоритет ЖК Мой Ритм, не выше 7 этажа". Объявление: "Сдам в ЖК Мой Ритм, 5/9 этаж, БЖ".
→ {"matches": true, "confidence": "high", "reason": "ЖК совпадает с приоритетом, этаж в пределах. Запрет на питомцев не противоречит notes."}

Пример 13.
Клиент: Виктор. Notes: "Один, без вредных привычек". Объявление: "Только семьи, БД БЖ".
→ {"matches": false, "confidence": "high", "reason": "В notes 'Один' (не семья), в объявлении 'Только семьи' — прямой конфликт."}

Пример 14.
Клиент: Лиля. Notes: (пусто). Объявление: текст без требований к жильцам.
